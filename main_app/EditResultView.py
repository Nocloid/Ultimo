...
class EditResultView(View):
    def get(self, request, *args, **kwargs):
        ...
        resultForm.fields['subject'].queryset = Subject.objects.filter(docente=docente)
        context = {
            'form': resultForm,
            'page_title': "Editar el resultado del estudiante"
        }
        return render(request, "staff_template/edit_student_result.html", context)

    def post(self, request, *args, **kwargs):
        form = EditResultForm(request.POST)
        context = {'form': form, 'page_title': "Editar el resultado del estudiante"}
        if form.is_valid():
            try:
                ...
                messages.warning(request, "El resultado no se pudo actualizar")
        else:
            messages.warning(request, "El resultado no se pudo actualizar")
        return render(request, "staff_template/edit_student_result.html", context)
